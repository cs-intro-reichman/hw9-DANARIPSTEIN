...
public class MemorySpace {
	...
	public int malloc(int length) {
		for (int i = 0; i < this.freeList.getSize(); i++) {
			...
			if (length < currentBlock.length) {
				MemoryBlock m1 = new MemoryBlock(currentBlock.baseAddress, length);
	...
	public void free(int address) {
		...
		for (int i = 0; i < this.allocatedList.getSize(); i++) {
			MemoryBlock allocatedBlock = this.allocatedList.getBlock(i);
			if (address == allocatedBlock.baseAddress) {
				this.freeList.addLast(allocatedBlock);
				this.allocatedList.remove(i);
				return;
			}
		}
	}
	...
	/**
	 * A textual representation of the free list and the allocated list of the
	 * memory space,
	...
	public void defrag() { 
		for (int i = 0; i < this.freeList.getSize(); i++) {
			MemoryBlock current = this.freeList.getBlock(i);
			for (int j = 0; j < this.freeList.getSize(); j++) {
				MemoryBlock next = this.freeList.getBlock(j);
